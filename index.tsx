...
// --- LAYOUT COMPONENTS ---
const Header = ({ onNavigate, onNewDiagnosis, onViewReports, theme, onToggleTheme }: {
    ...
    <header className="header shadow-md">
        <nav className="container mx-auto px-6 py-4 flex justify-between items-center">
            <div className="flex items-center space-x-3 cursor-pointer" onClick={() => onNavigate('home')}>
                <svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8 text-blue-600 dark:text-teal-400" viewBox="0 0 20 20" fill="currentColor">
                  <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm1-11a1 1 0 10-2 0v2H7a1 1 0 100 2h2v2a1 1 0 102 0v-2h2a1 1 0 100-2h-2V7z" clipRule="evenodd" />
                </svg>
                <span className="text-2xl font-bold text-gray-800 dark:text-gray-200">MediPredict AI</span>
            </div>
            <div className="flex items-center space-x-4">
                 <div className="hidden md:flex items-center space-x-8">
                    <a href="#" onClick={(e) => { e.preventDefault(); onNavigate('home'); }} className="text-gray-700 dark:text-gray-300 hover:text-blue-600 dark:hover:text-teal-400 transition font-medium text-lg">Home</a>
                    <a href="#" onClick={(e) => { e.preventDefault(); onNewDiagnosis(); }} className="text-gray-700 dark:text-gray-300 hover:text-blue-600 dark:hover:text-teal-400 transition font-medium text-lg">New Diagnosis</a>
                    <a href="#" onClick={(e) => { e.preventDefault(); onViewReports(); }} className="text-gray-700 dark:text-gray-300 hover:text-blue-600 dark:hover:text-teal-400 transition font-medium text-lg">View Reports</a>
                </div>
                 <button
                    onClick={onToggleTheme}
                    className="p-2 rounded-full text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition"
...
const Footer = ({ visitorCount, onProvideFeedback }: { visitorCount: number; onProvideFeedback: () => void; }) => (
    <footer className="bg-gray-800 dark:bg-black text-white text-center p-6 mt-auto">
        <div className="container mx-auto">
            ...
            <div className="flex justify-center items-center space-x-4 flex-wrap">
                 ...
                 <button 
                    ...
                    className="text-sm text-gray-400 dark:text-gray-500 hover:text-white dark:hover:text-gray-300 hover:underline transition"
                 ...
                 <span className="text-gray-500 dark:text-gray-600 hidden sm:inline">|</span>
                 <p className="text-sm text-gray-400 dark:text-gray-500">Page Visitors: {visitorCount}</p>
            </div>
        </div>
    </footer>
);
...
const Feature = ({ icon, title, description }: { icon: React.ReactElement, title: string, description: string }) => (
    <div className="text-center p-4">
        <div className="flex items-center justify-center h-12 w-12 rounded-full bg-blue-100 dark:bg-teal-900 text-blue-600 dark:text-teal-300 mx-auto mb-4">
            {icon}
        </div>
    ...
    <div className="w-full">
        <div className="card text-center p-8 md:p-12 max-w-4xl mx-auto mt-16">
            ...
            <div className="mt-8 flex flex-col sm:flex-row gap-4 justify-center">
                ...
                <button onClick={onViewReports} className="bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300 font-bold py-3 px-8 rounded-full text-lg shadow-lg w-full sm:w-auto hover:bg-gray-300 dark:hover:bg-gray-600 transition">
                    View Past Reports
                </button>
            </div>
        </div>
...
const PatientForm = ({ onSubmit, onBack }: { onSubmit: (patient: Patient) => void; onBack: () => void; }) => {
    ...
    return (
        <div className="card w-full max-w-lg p-8">
            <button onClick={onBack} className="text-blue-500 dark:text-teal-400 hover:underline mb-4">&larr; Back to Home</button>
            <h2 className="text-2xl font-bold text-center text-gray-800 dark:text-gray-200 mb-6">Patient Information</h2>
            <form onSubmit={handleSubmit} className="space-y-4" noValidate>
                <div>
                    <input type="text" name="name" placeholder="Full Name" value={patient.name} onChange={handleChange} className={`w-full p-3 border rounded-lg dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white ${errors.name ? 'input-error' : 'border-gray-300'}`} required />
                    {errors.name && <p className="error-text">{errors.name}</p>}
                </div>
                <div>
                    <input type="number" name="age" placeholder="Age" value={patient.age} onChange={handleChange} className={`w-full p-3 border rounded-lg dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white ${errors.age ? 'input-error' : 'border-gray-300'}`} required />
                    {errors.age && <p className="error-text">{errors.age}</p>}
                </div>
                <div>
                    <select name="gender" value={patient.gender} onChange={handleChange} className={`w-full p-3 border rounded-lg dark:bg-gray-700 dark:border-gray-600 dark:text-white ${errors.gender ? 'input-error' : 'border-gray-300'}`} required>
                ...
                <div>
                    <input type="tel" name="contact" placeholder="Contact (Optional)" value={patient.contact} onChange={handleChange} className={`w-full p-3 border rounded-lg dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white ${errors.contact ? 'input-error' : 'border-gray-300'}`} />
                    {errors.contact && <p className="error-text">{errors.contact}</p>}
                </div>
                <div>
                    <input type="email" name="email" placeholder="Email (Optional)" value={patient.email} onChange={handleChange} className={`w-full p-3 border rounded-lg dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white ${errors.email ? 'input-error' : 'border-gray-300'}`} />
...
const SymptomSelector = ({ onPredict, onBack, isLoading }: { onPredict: (symptoms: string[], image: string | null) => void; onBack: () => void; isLoading: boolean; }) => {
    ...
    return (
        <div className="card w-full max-w-4xl p-8 flex flex-col" style={{minHeight: '80vh'}}>
            <button onClick={onBack} className="text-blue-500 dark:text-teal-400 hover:underline mb-4 self-start">&larr; Back to Patient Form</button>
            <h2 className="text-2xl font-bold text-center text-gray-800 dark:text-gray-200 mb-2">Select Your Symptoms</h2>
            <p className="text-center text-gray-500 dark:text-gray-400 mb-4">Please select all symptoms you are experiencing.</p>
            
            <div className="relative">
                <input 
                    type="text"
                    placeholder="Search for a symptom..."
                    className="w-full p-3 border border-gray-300 dark:border-gray-600 rounded-lg mb-4 sticky top-4 bg-white dark:bg-gray-700 z-20"
                    value={searchTerm}
                    onChange={e => {
                ...
                {showSuggestions && suggestions.length > 0 && (
                    <ul 
                        id="suggestions-list"
                        className="absolute top-full -mt-4 w-full bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-b-lg shadow-lg z-30" 
                        role="listbox"
                    >
                        {suggestions.map(symptom => (
                            <li key={symptom}>
                                <button
                                    ...
                                    className="w-full text-left p-3 hover:bg-gray-100 dark:hover:bg-gray-700 cursor-pointer"
                                    role="option"
                                >
            ...
            <div className="border-t border-b border-gray-200 dark:border-gray-700 py-4 my-4">
                <h3 className="text-lg font-semibold text-gray-700 dark:text-gray-300 text-center mb-3">Add a Photo (Optional)</h3>
                <p className="text-center text-gray-500 dark:text-gray-400 mb-4 text-sm">If you have a visible symptom like a skin rash, add a photo for a more accurate prediction.</p>
                {image ? (
                    ...
                    <div className="flex justify-center">
                        <button onClick={handleImageUploadClick} className="flex items-center gap-2 bg-gray-200 dark:bg-gray-600 text-gray-700 dark:text-gray-300 font-bold py-2 px-6 rounded-full hover:bg-gray-300 dark:hover:bg-gray-500 transition">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
                    {filteredSymptoms.map(symptom => (
                        <div key={symptom}>
                            ...
                            <label
                                htmlFor={symptom}
                                className="block p-3 border border-gray-300 dark:border-gray-600 rounded-lg text-center cursor-pointer transition-colors duration-200"
                            >
                                {formatSymptom(symptom)}
                            </label>
                        </div>
            ...
            <button
                ...
                className="w-full mt-6 btn-primary text-white font-bold py-3 px-6 rounded-lg text-lg shadow-md disabled:bg-gray-400 dark:disabled:bg-gray-500"
                disabled={isLoading || selectedSymptoms.length < 2}
            >
                {isLoading ? 'Predicting...' : 'Predict Disease'}
            </button>
            {selectedSymptoms.length < 2 && <p className="text-center text-sm text-gray-500 dark:text-gray-400 mt-2">Please select at least 2 symptoms.</p>}
        </div>
    );
};
...
const ResultPageContent = ({ result, patient, symptoms, image }: { result: PredictionResult; patient: Patient, symptoms: string[], image: string | null }) => (
    <>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6 border-b border-gray-200 dark:border-gray-700 pb-6">
            <div>
                <h3 className="text-lg font-semibold text-gray-700 dark:text-gray-300 mb-2">Patient Details</h3>
            ...
            <div>
                <h3 className="text-lg font-semibold text-gray-700 dark:text-gray-300 mb-2">Reported Symptoms</h3>
                <div className="flex flex-wrap gap-2">
                    {symptoms.map(s => <span key={s} className="bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-200 text-sm font-medium px-3 py-1 rounded-full">{formatSymptom(s)}</span>)}
                </div>
            </div>
        </div>
        ...
        {image && (
            <div className="mb-6">
                <h3 className="text-lg font-semibold text-gray-700 dark:text-gray-300 mb-2">Symptom Image Provided</h3>
                <img src={image} alt="Symptom" className="rounded-lg shadow-md max-h-64 mx-auto"/>
            </div>
        )}

        <div className="bg-blue-50 dark:bg-teal-900/50 p-6 rounded-lg mb-6">
            <h3 className="text-xl font-semibold text-blue-800 dark:text-teal-200">Predicted Disease:</h3>
            <p className="text-2xl font-bold text-blue-900 dark:text-teal-100">{result.predictedDisease}</p>
             <p className="mt-2 text-gray-600 dark:text-gray-400">{result.description}</p>
        </div>

        <div className="grid md:grid-cols-3 gap-6 mb-6 text-left">
            <div className="bg-green-50 dark:bg-green-900/50 p-4 rounded-lg">
                <h4 className="font-bold text-green-800 dark:text-green-300 mb-2">Precautions</h4>
            ...
            <div className="bg-yellow-50 dark:bg-yellow-900/50 p-4 rounded-lg">
                <h4 className="font-bold text-yellow-800 dark:text-yellow-300 mb-2">Medications</h4>
            ...
            <div className="bg-red-50 dark:bg-red-900/50 p-4 rounded-lg">
                <h4 className="font-bold text-red-800 dark:text-red-300 mb-2">Diet</h4>
        ...
        <div className="bg-gray-100 dark:bg-gray-700/50 p-4 rounded-lg text-center text-sm text-gray-600 dark:text-gray-300 mb-6">
...
const ResultPage = ({ result, patient, symptoms, image, onDownload, onNewDiagnosis }: { result: PredictionResult; patient: Patient; symptoms: string[]; image: string | null; onDownload: () => void; onNewDiagnosis: () => void; }) => {
        ...
        return (
            <div className="card w-full max-w-4xl p-8 text-center">
                ...
                <div className="bg-red-50 dark:bg-red-900/30 p-6 rounded-lg mb-6">
                    <p className="text-lg text-red-800 dark:text-red-300">{result.description}</p>
                </div>
                <div className="mt-8">
    ...
    return (
        <div className="card w-full max-w-4xl p-8">
            <h2 className="text-3xl font-bold text-center text-gray-800 dark:text-gray-200 mb-6">Prediction Result</h2>
    ...
    <div className="modal-backdrop" onClick={onClose}>
        <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            <div className="flex justify-between items-center mb-4">
                <h2 className="text-2xl font-bold text-gray-800 dark:text-gray-200">Report Details</h2>
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">Report generated on {report.date}</p>
    ...
    return (
        <div className="modal-backdrop" onClick={onClose}>
            <div className="modal-content !max-w-lg" onClick={(e) => e.stopPropagation()}>
                <div className="flex justify-between items-center mb-4">
                    <h2 className="text-2xl font-bold text-gray-800 dark:text-gray-200">Provide Feedback</h2>
                    <button onClick={onClose} className="text-gray-500 dark:text-gray-400 hover:text-gray-800 dark:hover:text-gray-100 text-3xl font-light">&times;</button>
                </div>
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div>
                        ...
                        <select 
                            ...
                            className="w-full p-3 border border-gray-300 rounded-lg dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white"
                        >
                    ...
                    <div className="flex justify-end gap-4 pt-4">
                        <button type="button" onClick={onClose} className="bg-gray-200 dark:bg-gray-600 text-gray-700 dark:text-gray-200 font-bold py-2 px-6 rounded-lg hover:bg-gray-300 dark:hover:bg-gray-500 transition">
                            Cancel
                        </button>
    ...
    const renderContent = () => {
        switch(step) {
            case 'user_login':
                return (
                    <div>
                        <button onClick={() => { setStep('select_role'); resetForms(); }} className="text-blue-500 dark:text-teal-400 hover:underline mb-4 text-sm">&larr; Back to role selection</button>
                        <h3 className="text-xl font-bold text-center text-gray-800 dark:text-gray-200 mb-4">Patient Verification</h3>
                        <p className="text-center text-gray-500 dark:text-gray-400 mb-6 text-sm">Enter your details to view your personal reports.</p>
                        <form onSubmit={handleUserSubmit} className="space-y-4">
                            <input type="text" value={fullName} onChange={e => setFullName(e.target.value)} placeholder="Full Name" className="w-full p-3 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 rounded-lg" />
                            <input type="number" value={age} onChange={e => setAge(e.target.value)} placeholder="Age" className="w-full p-3 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 rounded-lg" />
            ...
            case 'admin_login':
                 return (
                    <div>
                        <button onClick={() => { setStep('select_role'); resetForms(); }} className="text-blue-500 dark:text-teal-400 hover:underline mb-4 text-sm">&larr; Back to role selection</button>
                        <h3 className="text-xl font-bold text-center text-gray-800 dark:text-gray-200 mb-4">Administrator Login</h3>
                        <p className="text-center text-gray-500 dark:text-gray-400 mb-6 text-sm">Enter your credentials to view all patient reports.</p>
                        <form onSubmit={handleAdminSubmit} className="space-y-4">
                            <input type="text" value={username} onChange={e => setUsername(e.target.value)} placeholder="Username" className="w-full p-3 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 rounded-lg" />
                            <input type="password" value={password} onChange={e => setPassword(e.target.value)} placeholder="Password" className="w-full p-3 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 rounded-lg" />
                            {error && <p className="error-text text-center">{error}</p>}
            ...
            default:
                return (
                    <div className="text-center">
                        <h3 className="text-xl font-bold text-gray-800 dark:text-gray-200 mb-4">View Past Reports</h3>
                        <p className="text-gray-600 dark:text-gray-400 mb-6">Please select your role to continue.</p>
                        <div className="flex flex-col gap-4">
                            <button onClick={() => setStep('user_login')} className="w-full bg-blue-100 dark:bg-teal-900/50 text-blue-800 dark:text-teal-200 font-bold py-4 px-6 rounded-lg text-lg shadow-sm hover:bg-blue-200 dark:hover:bg-teal-900/80 transition">I am a Patient</button>
                            <button onClick={() => setStep('admin_login')} className="w-full bg-gray-200 dark:bg-gray-700 text-gray-800 dark:text-gray-200 font-bold py-4 px-6 rounded-lg text-lg shadow-sm hover:bg-gray-300 dark:hover:bg-gray-600 transition">I am an Admin</button>
                        </div>
                    </div>
                );
        }
    ...
    return (
        <div className="modal-backdrop" onClick={onClose}>
            <div className="modal-content !max-w-md" onClick={(e) => e.stopPropagation()}>
                 <div className="flex justify-between items-center mb-4">
                    <h2 className="text-2xl font-bold text-gray-800 dark:text-gray-200">Authentication</h2>
                    <button onClick={onClose} className="text-gray-500 dark:text-gray-400 hover:text-gray-800 dark:hover:text-gray-100 text-3xl font-light">&times;</button>
                </div>
                {renderContent()}
            </div>
        ...
        <>
            <div className="card w-full max-w-4xl p-8">
                <div className="flex flex-wrap justify-between items-center mb-6 gap-4">
                    <div>
                        <button onClick={onBack} className="text-blue-500 dark:text-teal-400 hover:underline mb-2 block">&larr; Back to Home</button>
                        <h2 className="text-2xl font-bold text-gray-800 dark:text-gray-200">Past Reports</h2>
                    </div>
                    {reports.length > 0 && (
                        <button 
                ...
                {reports.length === 0 ? (
                    <p className="text-center text-gray-500 dark:text-gray-400">No past reports found.</p>
                ) : (
                    <div className="space-y-4">
                        {reports.map(report => (
                            <div key={report.id} className="p-4 border dark:border-gray-700 rounded-lg bg-gray-50 dark:bg-gray-800 flex justify-between items-center">
                                <div>
                                    <p className="font-semibold">{report.patient.name} - <span className="font-bold text-blue-700 dark:text-teal-400">{report.result.predictedDisease}</span></p>
...
const SuccessAnimation = () => (
    <div className="fixed inset-0 bg-white/75 dark:bg-gray-900/75 backdrop-blur-sm flex items-center justify-center z-50">
        <div className="text-center">
            <svg className="checkmark" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 52 52">
                <circle className="checkmark__circle" cx="26" cy="26" r="25" fill="none"/>
                <path className="checkmark__check" fill="none" d="M14.1 27.2l7.1 7.2 16.7-16.8"/>
            </svg>
            <p className="text-2xl font-semibold mt-4 text-gray-800 dark:text-gray-200 animate-pulse">Prediction Successful!</p>
...
const LoadingPage = ({ message }: { message: string }) => (
    <div className="card w-full max-w-md p-8 text-center flex flex-col justify-center items-center" style={{minHeight: '50vh'}}>
        <div className="loader ease-linear rounded-full border-8 border-t-8 border-gray-200 dark:border-gray-700 h-24 w-24 mb-6"></div>
        <h2 className="text-2xl font-bold text-gray-800 dark:text-gray-200 mb-2">Generating Report</h2>
        <p className="text-gray-600 dark:text-gray-400 animate-pulse min-h-[48px] flex items-center">{message}</p>
    </div>
);
...
const App = () => {
    ...
    return (
        <div className="flex flex-col min-h-screen">
            <Header onNavigate={setPage} onNewDiagnosis={handleNewDiagnosis} onViewReports={handleViewReports} theme={theme} onToggleTheme={toggleTheme} />
